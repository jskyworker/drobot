...
def post():
    ...
    for i in data:
        h = hashlib.md5(s.substitute(name=i['name'], date=i['date'], uid=i['uid']).encode('utf-8'))
        if h.hexdigest() != i['md5checksum']:
            return "Provided checksum {0}, should be {1} for name {2}.".format(i['md5checksum'], h.hexdigest(), i['name'])
        else:
            if not store_mongo(i):
                return "Checksum was correct but data was not saved."
            return "Correct checksum {0} for name {1}".format(h.hexdigest(), i['name'])


def get_num(uid, date):
    """
    Given a uid and a date the endpoint should return the number of occurrences of a given UID (John Doe) for that day.
    :param uid:
    :param date:
    :return:
    """
    ...
    with open('data.json', 'r') as f:
        data = json.load(f)
    for i in data:
        if date_c == datetime.strptime(i['date'], '%Y-%m-%dT%H:%M:%S.%f').date() and uid == i['uid']:
